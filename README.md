...
- 默认欢迎页（精简版）：ASCII 标题 + 网络 IP/Proxy + AI 快速一览（Claude model/24h/7d · Codex model/5h limit/weekly limit · Copilot plan/quota/sessions）；TTY 退化判断与显式 `welcome` 命令均已就绪
...
- Copilot 用量采集已对接本地 `copilot-api` 服务（`localhost:1414/usage`），展示 plan 类型、premium 配额（已用/总量/百分比）、chat/completions 是否 unlimited、配额重置日期；API 不可用时回退到本地 session 扫描。端口可通过 `PUP_COPILOT_API_PORT` 环境变量配置
